...
def build_clf(unit, input_shape):
    model = Sequential()
...
clf = KerasClassifier(build_fn=build_clf, epochs=500, input_shape=n_features)

params = {'unit': [8, 16, 32, 64], 
          }
...
sns.lineplot(data=history.cv_results_['mean_test_score'], ax=ax[0])
...
sns.lineplot(data=history.cv_results_['std_test_score'], ax=ax[1])
